<script lang="ts">
    ...
    async function pruneBackups() {
        try {
            const deletedCount = await invoke<number>("prune_backups", {
                keepCount: keepBackupCount,
            });
...
<div class="space-y-6">
        ...
        <div class="space-y-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    {#if $backupSuccess}
                        <div
                            class="mt-2 text-green-600 dark:text-green-400 text-sm flex items-center"
                        >
                    ...
                    {#if $backupError}
                        <div
                            class="mt-2 text-red-600 dark:text-red-400 text-sm flex items-center"
                        >
                            <FontAwesomeIcon
                ...
                <div class="text-sm text-gray-600 dark:text-gray-400">
                    {backupCount}
                    {backupCount === 1 ? "backup" : "backups"} available
                </div>
            </div>
    ...
    <section class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
        ...
        {#if $backups.length === 0}
            <div
                class="p-4 bg-gray-50 dark:bg-gray-800 rounded-md text-center text-gray-500 dark:text-gray-400"
            >
                No backups available yet. Create your first backup to protect
                your notes.
            </div>
        {:else}
            <div class="grid grid-cols-1 gap-2 mb-4">
                {#each $backups as backup}
                    <div
                        ...
                        class:dark:bg-blue-900={$selectedBackupPath ===
                            backup.path}
                        class:dark:bg-opacity-20={$selectedBackupPath ===
                            backup.path}
                        class:border-gray-200={$selectedBackupPath !==
                            backup.path}
                        class:dark:border-gray-700={$selectedBackupPath !==
                            backup.path}
                        class:border-blue-300={$selectedBackupPath ===
                            backup.path}
    ...
    <div
        class="bg-yellow-50 dark:bg-yellow-900 dark:bg-opacity-20 rounded-md p-4 border border-yellow-100 dark:border-yellow-800 mt-4"
    >
        <h3
        ...
        <p class="text-sm text-yellow-700 dark:text-yellow-400">
            Restoring a backup will replace all your current notes with the
            content from the backup. Make sure to create a new backup of your
            current state before restoring if you want to keep your changes.
        </p>
    </div>
</div>
